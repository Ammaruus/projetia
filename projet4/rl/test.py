...
class LabyrinthGame:
    ...
    def play(self, actions):
        self.env.reset()
        self.env.render()
        time.sleep(5)  # Pause initiale pour le premier rendu
        for action in actions:
            ...
            self.env.render()
            time.sleep(2)  # Pause de 5 secondes après chaque mouvement
        cv2.destroyAllWindows()


# Actions: 0 - Nord, 1 - Est, 2 - Sud, 3 - Ouest
actions = [1, 1, 1, 0, 0, 0, 3, 3, 3, 2, 2,2,2]  # Exemple d'actions
game = LabyrinthGame()
game.play(actions)
